                ...
                if (Path.Count == 2)
                {
                    try
                    {
                        ...
                        if (Path.Contains("y = a^b mod p"))
                        {
                            ...
                            await client.SendTextMessageAsync(
                                 chatId: msg.Chat.Id,
                                 String.Format("{0}^{1} (mod {2}) = {3}", newdata[0], newdata[1], newdata[2], answer),
                                 replyMarkup: GeneralMenu());
                        }
                        if (Path.Contains("y = a*b mod p"))
                        {
                        ...
                        if (Path.Contains("Минимальная циклическая группа"))
                        {
                            ...
                            await client.SendTextMessageAsync(
                                 chatId: msg.Chat.Id,
                                 String.Format("{0} - циклическая группа поля {1}", answer, newdata[0]),
                                 replyMarkup: GeneralMenu());
                        }
                        if (Path.Contains("Метод тривиальных(простых) делений"))
                        {

                            var newdata = Equations.MethodTrialDiv(int.Parse(msg.Text));
                            var str = "";
                        ...
                        if (Path.Contains("Метод Ферма"))
                        {
                            ...
                            var newdata = int.Parse(msg.Text);
                        ...
                        if (Path.Contains("Обратный элемент"))
                        {
                            ...
                            await client.SendTextMessageAsync(
                                chatId: msg.Chat.Id,
                                String.Format("{0}*x=1 (mod {1}), x = {2}", newdata[0], newdata[1], answer),
                                replyMarkup: GeneralMenu());
                        }
                    }
